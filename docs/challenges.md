...
| **Audit trail** | `backend/compliance/audit.py` | Every access to sensitive resources is logged: thumbnail access, feedback submissions, active-learning exports, chat queries, agent invocations. Each record includes timestamp, action, resource, outcome, and IP. GDPR Art. 30 / SOC 2 ready. |
...
| **Privacy compliance** | EU/GDPR: mature enforcement. US/BIPA: active litigation around biometric capture | Dual thumbnails, plate hashing at ingest, audit trail, auto-retention | `redact.py`, `audit.py`, `retention.py` |
