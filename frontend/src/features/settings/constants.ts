...
export const TUNABLE_HELP: Record<string, TunableHelp> = {
  CONF_THRESHOLD: {
    ...
    decrease:
      "More detections including marginal ones. Adds load to LLM enrichment and downstream gates; may admit noise that scene-context can't fully suppress.",
  },
  PERSON_CONF_THRESHOLD: {
    ...
    increase:
      "Fewer false-positive 'person' boxes from occlusion bleed. Risks missing distant or partially-occluded pedestrians (which legitimately score lower).",
    decrease:
      "Catches more distant / partially-visible people. Below ~0.25 with YOLOv8n it lets occasional noise through; pair with a larger model variant if you go aggressive.",
  },
  VEHICLE_PAIR_CONF_FLOOR: {
    ...
    decrease:
      "More vehicle-pair alerts. Use only if your camera reliably yields high-confidence vehicle bboxes.",
  },
  MIN_BBOX_AREA: {
  ...
  TTC_HIGH_SEC: {
    ...
    decrease:
      "Stricter HIGH classification. Fewer Slack high-tier alerts; only the most imminent collisions fire.",
  },
  TTC_MED_SEC: {
  ...
  MIN_SCALE_GROWTH: {
    ...
    decrease:
      "TTC fires for smaller scale changes; more sensitivity, more jitter-driven false alerts. Below 1.05 you start emitting TTC for bbox noise alone.",
  },
  TRACK_HISTORY_LEN: {
    what: "Number of trailing samples kept per tracked object for TTC math.",
    affects:
      "TTC stability and responsiveness; longer history smooths estimates and is required for sustained-growth detection.",
    increase:
      "More stable TTC estimates and longer lookback for sustained-growth checks. Slightly more memory per active track and slower adaptation to new fast-moving objects.",
    decrease:
      "Faster reaction to brand-new tracks. Risks falling below the multi-gate sustained-growth sample-count requirement, which silently disables TTC for short-lived tracks.",
  },
  QUALITY_BLUR_SHARP: {
    what: "Laplacian-variance value below which the camera is classified as blurred (dirty lens / motion blur / fog).",
    affects:
      "QualityMonitor state machine — degraded states suppress event emission and widen TTC thresholds.",
    increase:
      "Stricter sharpness requirement; pipeline degrades into 'blurred' more often, suppressing events. Use when the lens is reliably clean and you want extra safety on borderline frames.",
    decrease:
      "More permissive — runs through more frames without suppression but admits blurry false positives.",
  },
  ...
  LLM_BUCKET_CAPACITY: {
    ...
    affects:
      "How many LLM calls can fire back-to-back during an event burst before throttling kicks in.",
    increase:
      "Larger LLM bursts during event clusters. Higher peak cost and risk of 429s if you exceed the provider's rate limit.",
    decrease:
      "More aggressive throttling — non-essential narration / enrichment skipped during bursts. Lower cost, more 'rate budget exhausted' skip records in /api/llm/stats.",
  },
  LLM_BUCKET_REFILL_PER_MIN: {
    ...
    increase:
      "Higher sustained LLM call rate, higher cost. Stay below the provider's rate limit (Anthropic Haiku low-tier ≈ 5 req/min).",
    decrease:
      "Lower sustained call rate; more events get the deterministic narration fallback rather than an LLM-generated one.",
  },
  ...
  ALPR_MODE: {
    ...
    options: {
      ...
      on_demand:
        "ALPR only fires when an event is explicitly flagged for review. Balanced posture; cost scales with operator review rate, not event rate.",
    },
  },
  PAIR_COOLDOWN_SEC: {
    what: "After emitting an event for a (track A, track B) pair, suppress further events from the same pair for this many seconds.",
    affects:
      "Event-stream noise — without this a single sustained near-miss would emit ~20 events.",
    increase:
      "Fewer duplicate events; cleaner stream during sustained interactions. Risks coalescing two genuinely separate close-calls between the same pair into one report.",
    decrease:
      "More granular reporting on evolving incidents. Risks spamming the SSE stream and Slack on long sustained near-misses.",
  },
  MAX_RECENT_EVENTS: {
  ...
  TARGET_FPS: {
    ...
    decrease:
      "Lower compute load and LLM headroom. Risks missing sub-second TTC events. Below 1 fps the multi-gate sustained-growth requirement starts struggling for sample volume.",
  },
};
