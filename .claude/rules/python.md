---
name: python
description: Python conventions for backend/, cloud/, tests/, start.py
type: rules
paths:
  - "**/*.py"
---
...
- **Paths**: never compute `Path(__file__).parent` in modules. Import paths from [backend/config.py](backend/config.py) — single source of truth.
...
- **Error handling**: prefer narrow `except` clauses. Bubble unexpected errors to the watchdog ([backend/services/watchdog.py](backend/services/watchdog.py)) — do not swallow.

## Hot-path rules (perception pipeline)

- Do not short-circuit conflict-detection gates in [backend/core/](backend/core/) — each kills a specific false-positive class.
...
- Do not leak raw plate text. Scrub at ingest in `enrich_event()` ([backend/services/llm.py](backend/services/llm.py)), not just at egress.
